"""Torus manifold utilities: geometry, metrics, config, action samplers, color/position ground truth.
...
The `TorusEnv` WorldEnv contract implementation lives in `environments/examples/torus.py`
and imports everything env-shaped from here.
...
# --------------------------------------------------------------------------------------
# Environment config (the dynamics knobs; the env class itself lives in examples/torus.py)
# --------------------------------------------------------------------------------------
@dataclass
class TorusConfig:
