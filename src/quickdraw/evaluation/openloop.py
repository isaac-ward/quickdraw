...
def proprio_curves(preds_norm, true_norm, p_hat, p_true, R, r, v_scale):
    """The 4 open-loop proprio per-step metrics (each (N,H)), shared by eval_batched (vector spine) and
    ...
    return {
        ...
        "manifold_distance_error": T.manifold_distance_error(p_hat, R, r),      # dimensionless (÷ r)
        "pointwise_error": T.pointwise_error(p_hat, p_true),            # ‖p̂ − p‖ (physical units)
        "tangent_velocity_error": T.tangent_velocity_error(p_hat, R, v_scale),  # dimensionless (÷ v_scale)
    }
...
def eval_batched(model, normalizer, R, r, v_scale, P, obs_seq, act_seq):
    ...
    per_step = proprio_curves(preds, obs_seq[:, P:], p_hat, p_true, R, r, v_scale)
