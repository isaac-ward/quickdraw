...
COMMON=( model=mm_flow_proprio data.root="$DATA" data.batch=256 model.detach_every=16  # 1 run/GPU -> autobatch (default ON) sizes the batch; data.batch=256 is only the fallback if autobatch is disabled
