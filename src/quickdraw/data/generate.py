...
def write_lerobot_split(root, repo_id: str, obs, act, fps: int,
                        fpv_dir: str | list[str] | None = None, fpv_size=256,
                        cam: str | list[str] = "fpv", task: str = "torus"):
    ...
    # N CAMERAS: `cam`/`fpv_dir`/`fpv_size` may each be a single value (one camera, the original path,
    # bit-identical) or PARALLEL lists, one entry per camera. Each camera becomes its own
    # observation.images.<cam> video feature and its own per-frame entry.
    cams = [cam] if isinstance(cam, str) else list(cam)
    dirs = ([fpv_dir] if isinstance(fpv_dir, str) else list(fpv_dir)) if video else []
    szs = fpv_size if isinstance(fpv_size, list) else [fpv_size] * len(cams)
    if video and not (len(cams) == len(dirs) == len(szs)):
        raise ValueError(f"cam/fpv_dir/fpv_size must be parallel: {len(cams)}/{len(dirs)}/{len(szs)}")
    ...
    if video:   # vector-only splits pass fpv_dir=None and no fpv_size -> no image feature
        for k, sz in zip(keys, szs):
            h, w = (sz, sz) if isinstance(sz, int) else tuple(sz)
            features[k] = {"dtype": "video", "shape": (h, w, 3),
                           "names": ["height", "width", "channels"]}
    ds = LeRobotDataset.create(repo_id=repo_id, fps=fps, root=root, features=features, use_videos=video)
    for i in range(len(obs)):
        # one decoded clip per camera for THIS episode; held only for this episode's rows
        per_cam = [_read_frames(os.path.join(d, f"ep_{i:04d}.mp4")) for d in dirs] if video else []
        for t in range(len(obs[i])):
            f = {"observation_vector": obs[i][t], "action": act[i][t], "task": task}
            for k, fr in zip(keys, per_cam):
                f[k] = fr[t]
            ds.add_frame(f)
        del per_cam                                  # 3 cameras x a long episode is GBs; drop before the next
        ds.save_episode()
    ds.finalize()
    return ds
