...
renders the recorded sticks with build_action_text. That makes this row NOT comparable to a per-step
row: the model is told the intent, not the signal.
    ...
    def _rollout(self, ctx: dict[str, Tensor], actions, horizon: int, heads) -> dict[str, Tensor]:
        ...
        while min(len(g) for g in gen) < horizon:
            for lo in range(0, n, self.batch):
                ...
                out = pipe(video=list(cond), prompt=prompts[lo:hi],
