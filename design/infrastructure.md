...
services:
  app:
    ...
    command: uv run python -m quickdraw.train
volumes:
  caches:
```
...
uv run python -m quickdraw.train          experiment=$RUN              # val + in-dist open-loop
...
```
Config overrides pass straight through, e.g. `... quickdraw.train model.depth=6 trainer.max_epochs=200`.
