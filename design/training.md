...
conf/
  ...
  model/base.yaml        # d, depth, heads, window W, p_tf
  ...
  env/torus.yaml         # R, r, dt, gamma, a_max  (shared by generation + control)
  logging/wandb.yaml     # project, group, cadence
  eval/default.yaml      # rollout horizon + during_train: {every_epochs, run: [long_horizon|ood|control]}
...
- `LightningModule` wraps `BaseWorldModel`; `Trainer(max_epochs, precision="bf16-mixed",
  gradient_clip_val=1.0, check_val_every_n_epoch=1)` → **train one epoch, then validate**.
...
- `validation_step`: windowed val loss/metrics. Separately, `EvalCallback` runs the **subscribed**
  eval routines (`cfg.eval.during_train.run`) every `every_epochs` — train/val are unaffected.
...
`data_generation` → `train` (train/val + subscribed in-loop evals) → standalone `eval_long_horizon` /
...
- **During training** — `EvalCallback` runs `cfg.eval.during_train.run` (a subset of the three) every
  `every_epochs`. `0` or `[]` disables.
- **Standalone at best ckpt** — `quickdraw.eval_long_horizon | eval_ood | eval_control`, each loading
...
W `θ=π`, S `θ=3π/2`. Named e.g. `outer-N`, `top-E`, `inner-S`, `bottom-W`.
...
`max_steps`. All `num_samples` and all 16 targets are batched into one GPU model call.
...
**Acceleration.** `num_samples × 16` rollouts batched through the compiled bf16 model; Hz reported
